...
import { CurrentCardType, TypeSetCover } from '@/components'
...
export const useCardModal = (
  ...
  const { id } = useParams()
  const [imgQuestionCover, setImgQuestionCover] = useState<TypeSetCover>(undefined)
  const [imgAnswerCover, setImgAnswerCover] = useState<TypeSetCover>(undefined)
